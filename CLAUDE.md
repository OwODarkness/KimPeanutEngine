...
- **Known RHI leaks (fixed 2026-08-15/16):** shaders now arrive as `data::ShaderData` (`PipelineDesc` holds `data::ShaderData*`; the `graphics::Shader`/`ResourceShader`/`ShaderLoader` seams retired); the path-keyed `ShaderManager` (`shader_manager.*`, `shader_factory.h`, `vulkan_shader.*`, `opengl_shader.*`) is **retired and deleted**. `VulkanBackend::CreateGraphicsPipeline` and `OpenglBackend::CreatePipeline` now **take** the `PipelineDesc` and bake it. The build-time `glslc` → `.spv` step is **gone** (2026-08-16) — the `rhi_example` bakes its shaders at runtime via `ProcessShader` — and the stale `ShaderModule` seam is **deleted** (2026-08-16): the raw `ShaderData` → API-object step stays inline in the pipeline bakes (`VulkanPipelineManager::CreateShaderModule`, `OpenglPipeline`'s compile/attach/link).
...
- **The resource pipeline has a caller, but not the render module:** `ResourcePipeline::ProcessShader` is called by the `rhi_example` demo (2026-08-16) and the asset example. The render-module request path and a startup warmup pass (manifest → `LoadSync` + `ProcessShader`) are the reconstruction's first wiring step.
