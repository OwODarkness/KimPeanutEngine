...
5. **The debate argues for Vulkan as primary.** "Use one API" (Italink) and "one primary dev path" (skeptic) both support treating Vulkan as the reference backend and GL as the port — consistent with the current split: callers build descriptions and the backend bakes them through `CreatePipelineResource`.
