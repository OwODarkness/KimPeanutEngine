...
This is what makes `Render` ↔ `Graphics` wiring ([TODO 5.1](TODO.md)) meaningful, and it landed with the phase: `Render` now links `Graphics` (PRIVATE) — `RenderSystem` itself stays API-agnostic, only the demo scene records through the RHI. Build green, 86/86 tests. *Not re-run here: the demo window — verify visually that the triangle still draws.*
