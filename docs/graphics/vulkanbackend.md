...
**Snapshot: 2026-08-20.** A structural review of `VulkanBackend` — the original 2,149-line version is archived at [`backend/vulkan/deprecated/`](../../engine/runtime/graphics/backend/vulkan/deprecated/) — and a phased plan to peel the facade. Phases 0–5 are **landed** (see below); the current backend is the Phase-1/2/3/4/5 reconstruction ([vulkan_backend.h](../../engine/runtime/graphics/backend/vulkan/vulkan_backend.h), [vulkan_backend.cpp](../../engine/runtime/graphics/backend/vulkan/vulkan_backend.cpp), [vulkan_device.h](../../engine/runtime/graphics/backend/vulkan/vulkan_device.h), [vulkan_device.cpp](../../engine/runtime/graphics/backend/vulkan/vulkan_device.cpp), [vulkan_swapchain.h](../../engine/runtime/graphics/backend/vulkan/vulkan_swapchain.h), [vulkan_swapchain.cpp](../../engine/runtime/graphics/backend/vulkan/vulkan_swapchain.cpp), [vulkan_frame_context.h](../../engine/runtime/graphics/backend/vulkan/vulkan_frame_context.h), [vulkan_frame_context.cpp](../../engine/runtime/graphics/backend/vulkan/vulkan_frame_context.cpp)). The demo that used to live inside the backend is now [`render::RenderScene`](../../engine/runtime/render/render_scene.h) — the render module's first real scene. Companion to [graphics_module.md](graphics_module.md) (the RHI contract), [TODO.md](TODO.md) (the working ledger), and [sakura_reference.md](sakura_reference.md) / [rhi_design_material.md](rhi_design_material.md) (the design material). This page is the *how* for the Vulkan backend specifically; the ledger items stay authoritative for what.

## Verdict in one paragraph

`VulkanBackend` is a **correct resource layer under a god-object facade**. The resource ownership is genuinely split out — `VulkanBufferManager` (handle + allocator-strategy memory), `VulkanPipelineManager` (bakes a `PipelineDesc`), `VulkanImageMemoryManager`, and the cross-API `TextureManager`/`SamplerManager`/`MeshManager` — and that is the model the rest should follow. But *everything above resource creation* collapsed into one class: device lifecycle, swapchain, command pools, sync objects, image transitions, queue-ownership transfers, the per-frame UBO/descriptor wiring, **and the actual scene content** (one hardcoded mesh, one pipeline, one camera demo) all fused into ~2,100 lines (the original, archived at [`backend/vulkan/deprecated/vulkan_backend.cpp`](../../engine/runtime/graphics/backend/vulkan/deprecated/vulkan_backend.cpp)) that read asset paths from disk. The fix is not a rewrite — it is peeling the facade, phase by phase, with the current demo as the regression test until the render module can take over. **Phases 0–5 (shaders → `ShaderData`; `VulkanDevice`, `VulkanSwapchain`, `VulkanFrameContext` extraction; scene recording extracted; facade cleanup) are landed**; the line references below point at the archived original, where the numbers are still exact.
...
- [x] `VulkanBackend::CreatePipelineResource(PipelineDesc)` takes a caller-built desc and returns an independent `PipelineHandle` ([TODO](TODO.md)). The backend bakes only; a swapchain-bound desc gets its attachment formats completed from the swapchain format (an RHI-owned invariant). The caller owns stage/layout/bindings.
...
**Landed:** `PipelineDesc` holds `data::ShaderData*` (`byte_code` for Vulkan, `source` for OpenGL — the resource pipeline's artifact *is* the RHI's input, no wrapper). Both backends read the field their own API needs and touch no shader files. `RenderBackend::Initialize(WindowHandle)` creates only backend/frame state; callers later create and destroy any number of pipelines through `CreatePipelineResource(PipelineDesc)`. The demo creates two handles from caller-built descriptions. Since 2026-08-16 those `ShaderData`s are baked at runtime: `LoadSync(simple_triangle.shader)` → `ResourcePipeline::ProcessShader` fills `byte_code`/`source` per API — the build-time `glslc` step ([TODO 2.3](TODO.md)) is gone.
...
### Phase 4 — Extract scene recording ✅ landed 2026-08-15 (the behavior change — last on purpose)

The backend now exposes **where the frame is**, not what's in it: `BeginFrame()` acquires, transitions and `vkCmdBeginRendering`s the frame's scene command buffer; a caller records draws against it (`GetCurrentSceneCommandBuffer()`); `EndFrame()` ends rendering, submits and presents. All scene content is gone from the backend — `SetupResource` (`wallpaper.jpg` + `sphere.obj` load, staging, transitions), `CreateVertexBuffers`, `CreateUniformBuffers`/`CreateUniformBuffer`, `CreateDescriptorPool`/`CreateDescriptorSets`, `UpdateUniformBuffer`, `CopyBufferToImage` and `RecordCommandBuffer` were deleted; `per_pass_ubo_`/`per_object_ubo_`/`descriptor_sets_` are no longer backend members.

The demo reappears as [`render::RenderScene`](../../engine/runtime/render/render_scene.h) ([render_scene.cpp](../../engine/runtime/render/render_scene.cpp)) — the render module's first real scene. It owns the mesh (sphere), texture (wallpaper), per-frame UBOs and descriptor sets, drives the animated camera via the swapchain extent, and records the frame's draws as raw `vkCmd*` against the backend's scene command buffer. **Vulkan-specific for now** — a cross-API scene abstraction is the reconstruction's next step. The backend keeps the swapchain-bound depth/color attachments (RHI-sized resources) and a few scene-side facilities: `CreateUniformBuffer`/`MapUniformBuffer` and `UploadTexturePixels` (stage → one-shot copy → sample transition).

This is what makes `Render` ↔ `Graphics` wiring ([TODO 5.1](TODO.md)) meaningful. `Render` links `Graphics` (PRIVATE); since 2026-08-20 `RenderSystem` owns the API-neutral backend facade, pipeline warmup/cache, and frame bracket, while only the demo scene records concrete Vulkan commands. Build green, 86/86 tests. *Not re-run here: the demo window — verify visually that the triangle still draws.*

### Phase 5 — Facade cleanup ✅ landed 2026-08-16
...
The backend keeps owning GPU state (the managers) and the frame submission skeleton. It stops owning: shader *choice*, pipeline desc, swapchain/device lifecycle internals, and — most importantly — **what the scene draws** — which now lives above this shape as `render::RenderScene`, recording through the frame API. The shape above is the current backend as of Phase 4.
