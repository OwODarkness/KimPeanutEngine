...
- **Render issue-9.7 Sponza quality/throughput (2026-09-07)** — Stage 0
  ...
  Stage 6.0 telemetry now publishes those subphase timings and counters, Stage
  6.1 now stamps effective directional-shadow fits so inside-fit camera motion
  reuses the map, and Stage 6.2 now reuses stable geometry/material binding
  sets with dynamic uniform offsets; repeated unchanged editor viewport-size
  requests no longer reset the cache. Fixed-scenario Vulkan/OpenGL profile
  capture remains open before larger submission redesign. →
