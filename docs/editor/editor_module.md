...
- **`EditorLayoutState` / `editor_layout_settings.*`** — the persisted split fractions, keyed by splitter ID and stored in `save/editor_layout.json`. Tolerant on read: a missing, malformed, or unknown-version file yields defaults rather than stopping the editor.
