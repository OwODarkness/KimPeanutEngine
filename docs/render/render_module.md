...
- [x] Introduce a render-private pass interface or equivalent pass descriptor.
  ...
  2026-08-24 were superseded by R1.3's immutable fixed sequence, which R3.4c in
  turn replaced with the compiled graph plan; its logical resources remain
  render-owned names rather than RHI handles, and it names no `Vk*` objects,
  layouts, queues, or command buffers.
