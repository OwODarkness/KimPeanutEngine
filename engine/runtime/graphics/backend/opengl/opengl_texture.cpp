...
namespace kpengine::graphics
{
    void OpenglTexture::Initialize(GraphicsContext context, const TextureData &data, const TextureSettings &settings)
    {
        ...
        if (!data.pixels.empty())
        {
            if (settings.type == TextureType::TEXTURE_TYPE_2D || settings.type == TextureType::TEXTURE_TYPE_CUBE)
            {
                glTextureSubImage2D(resource_.image, settings.mip_levels, 0, 0, data.width, data.height, texture_cpu_format, texture_cpu_type, data.pixels.data());
            }
            else if (settings.type == TextureType::TEXTURE_TYPE_3D)
            {
                glTextureSubImage3D(resource_.image, settings.mip_levels, 0, 0, 0, data.width, data.height, data.depth, texture_cpu_format, texture_cpu_type, data.pixels.data());
            }
        }
