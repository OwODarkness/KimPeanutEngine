...
// harnesses and their includes are kept commented out below for reference —
// uncomment both together to recompile a standalone test.
// #include <memory>
// #include <iostream>
...
int main(int argc, char **argv)
{
    ...
    // non-zero instead of dying inside Initialize with no visible reason.
    try
    {
        engine.Initialize();
    }
    ...
    {
        std::fprintf(stderr, "KimPeanut Engine failed to initialize: %s\n", e.what());
        return 1;
    }
...
// ---------------------------------------------------------------------------
// [old design] Standalone test harnesses, preserved for reference. The engine
// boot above replaces the old "uncomment one test to run" entry point.
// ---------------------------------------------------------------------------
