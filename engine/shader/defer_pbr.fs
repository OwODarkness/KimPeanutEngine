...
void main()
{
    ...
    // Normalize the sampled normal (assuming stored as linear vector)
    vec3 normal_vec = normalize(texture(g_normal, uv).rgb);

    vec3 material = texture(g_material, uv).rgb;
    vec3 albedo = texture(g_albedo, uv).rgb;

    // Unpack material components
    float roughness = material.r; // roughness stored in R channel
    float metallic = material.g;  // metallic stored in G channel
    float ao = material.b;        // ambient occlusion stored in B channel

    // Calculate shadows
    float directional_shadow = CalculateShadowValue(vec4(0), normal_vec); // You should pass actual light_space coords here if you want directional shadow
    ...
    // Calculate attenuation and radiance for point light
    ...
    // Apply shadow from point light (for directional shadow, you need to incorporate directional_shadow here)
    float shadow = point_shadow; // + directional_shadow if available

 
    out_frag_color = vec4(color * (1.0 - shadow), 1.0);
}
